...
![alt text](https://github.com/ndeimler99/TArPON/tree/main/src/TArPON_Pipeline.png?raw=true)
...
In the simplest form TArPON can be run using the following command replacing "file" with the path of a gzipped fastq file or directory and NNNNNNNNNN with the appropriate adaptor sequence.
...
To enable demultiplexing an additional sample file will have to be provided
    ...
    nextflow run main.nf --input_file "file.fastq.gz" --adaptor NNNNNNNNNN --sample_file "sampleFile.csv"
...
| :---        |    :----:   | :----:   | :---: |       ---: |
| run_name      | Run Name |  Name of Sequencing Run for Overall Statistics  | String | String| Run | 
| input_file   | Input File |  FastQ File or Directory from Nanopore Sequencing for Analysis. If a directory, all fastq files included will be concatenated together  | File or Directory| ./data/test_fastq.fastq.gz |
...
| minimum_telo_reads_per_sample | Minimum Telo Reads Per Sample | Minimum number of telomeric reads per sample for the sample to pass filtering criteria and considered valid for drawing conclusions | Integer | 3000 |
...
|lower_threshold| Lower Threshold|The lower threshold (see upper threshold) at which a the proportion of one nucleotide variants of the telomeric repeat must not drop below in a sliding window | Float | 0.1|
...
|:--|:--:|:--:|:--:|
| specified output directory| report.html | HTML report with all relevant statistics and plots for easy viewing to open in your web browser | HTML |
| output/pipeline_info | execution_trace.txt | List of processes performed during the execution of the nextflow pipeline and their system requirements | txt file |
| | execution_timeline.html | Timeline of all processes showing memory usage and duration throughout pipeline execution| HTML |
| | execution_report.html | Detailed analysis on all pipeline executation statistics and commands | HTML |
| | pipeline_dag.html | Directed Acyclic Graph of Pipeline Execution showing how all processes interact | HTML |
| output/run_name_STATS | Filtered_Reads.stats.txt | Global Statistics on all reads removed from the analysis at various timepoints regardless of Multiplexing <br>Input.fastq.gz = Input Files<br>non_telomeric.fastq = Reads not Identified as Putatively Telomeric <br> 20_80_removed_reads.fastq = Reads removed due to inadequate identification of which strand the read originated<br>adaptor_filtered.fsatq = Reads that were not identified to contain an adaptor or barcode<br>subtelo_fail.fastq = Reads not Passing Subtelomeric Filtering Criteria<br>no_telo_start.fastq = Reads that the telomere start position was indeterminable<br>below_telo_threshold.fastq = Reads where the % of telomere composed of one nucleotide variants falls below telo_percent_threshold | txt file |
| | Retained_Reads.stats.txt | Global Statistics on all reads retained at each step throughout pipeline execution regardless of Multiplexing <br>Input.fastq.gz = Input Files<br>putative_reads.fastq = Reads putatively identified as telomeric<br>putative_reads.filtered.fastq = Reads Passing C/G Strand Filtering Criteria<br>adaptor.fastq = Reads where Adaptor and Barcode were properly identified<br>subtelo_pass.fastq = Reads where subtelomere existed<br>telo_retained.fastq = All Final Telomere Containing Reads| txt file|
|output/run_name_STATS/FIGURES/|filtered.length_boxplot.pdf| Boxplot dictating the length of reads filtered at each step described in Filtered_Reads.stats.txt |PDF|
|output/run_name_STATS/FIGURES/|filtered.length_boxplot_zoomed.pdf| Same as filtered.length_boxplot.pdf but with limited x-axis|PDF|
|output/run_name_STATS/FIGURES/|filtered.quality_score.pdf| Quality score boxplot of filtered reads at each step described in Filtered_Reads.stats.txt|PDF|
|output/run_name_STATS/FIGURES/|filtered.read_counts.pdf| Bar plot showing the number of reads filtered at each step of the pipeline|PDF|
|output/run_name_STATS/FIGURES/|filtered.read_counts.no_input.pdf| filtered.read_counts.pdf without the input file or non-telomeric reads|PDF|
|output/run_name_STATS/FIGURES/|retained.length_boxplot.pdf| Boxplot dictating the length of reads retained at each step as described in Retained_Reads.stats.txt|PDF|
|output/run_name_STATS/FIGURES/|retained.length_boxplot_zoomed.pdf|retained.length_boxplot.pdf with limited x-axis|PDF|
|output/run_name_STATS/FIGURES/|retained.quality_score.pdf|Quality score boxplot of reads retained at each step described in Retained_Reads.stats.txt|PDF|
|output/run_name_STATS/FIGURES/|retained.read_counts.pdf|Bar plot showing the number of reads retained at each step of the pipeline|PDF|
|output/run_name_STATS/FIGURES/|retained.read_counts.no_input.pdf|retained.read_counts.pdf with input removed |PDF|
| output/Sample_name/ |sample_name.filtered.stats.txt|All Statistics relating to this sample starting with demultiplexed reads<br>sample.fastq = Sample Specific Reads<br>sample.subtelo_fail.fastq = Sample Specific reads Failing Subtelomere Filtering Criteria<br>sample.no_telomere_start.fastq = Sample Specific Reads in which the Telomere Start was undefinable<br>sample.below_telo_%_threshold.fastq = Sample Specific reads in which the variant repeat rich region was not composed of the appropriate amount of one nucleotide variants|txt file|
| output/Sample_name/ |sample_name.retained.stats.txt| All Statistics relating to this sample starting with demultiplexed reads<br>sample.fastq = Sample Specific Reads<br>sample.subtelo_pass.fastq = Sample Specific Reads Pass Subtelomere Filtering Critera<br>sample.telomeric.fastq = Sample Specific Reads containing Filtered Telomeric Sequences|txt file|
| output/Sample_name/ |sample_name.telomeric.fastq|All reads containing finalized telomeric sequences|fastq file|
| output/Sample_name/FIGURES |read_length_hist.pdf| Read length histogram of reads used in final telomere analysis with mean read length dictated by a solid red vertical line |PDF|
| output/Sample_name/FIGURES |telo_length_bar_plot.pdf| Telomere length binned into 1kb bins for easy comparison between samples|PDF|
| output/Sample_name/FIGURES |telo_length_by_read_length.pdf| Density scatter plot showing relationship between telomere length and read length. If conducting a restriction digest during sequencing preparation there will most likely be a correlation between these two variables |PDF|
