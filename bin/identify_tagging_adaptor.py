...
min_subtelo_length = int(sys.argv[4])
min_subtelo_threshold = float(sys.argv[5])
repeat = sys.argv[6]
passes_subtelo = sys.argv[7]
adaptor_found = sys.argv[8]
fails_subtelo = sys.argv[9]
no_adaptor = sys.argv[10]

with open(input_file, 'r') as fh, open(passes_subtelo, 'w') as subtelo_out, \
 open(adaptor_found, 'w') as adaptor_out, open(fails_subtelo, 'w') as subtelo_fail, \
 open(no_adaptor, 'w') as adaptor_fail:
    read = []
    linecount = 0 
    for line in fh:
        linecount += 1
