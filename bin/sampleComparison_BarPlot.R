...
for (arg in args[3:length(args)]){
  if (args[1]){
    ...
    df$binned_telo_length <- unlist(lapply(df$telo_length, function(x) TELO_LENGTHS_FOR_BINNING[which.min(abs(TELO_LENGTHS_FOR_BINNING-x))]))
    ...
    telo_master_df <- rbind(telo_master_df, binned_telo_length)
  }
...
if (args[1]){
  telo_master_df$sample <- factor(telo_master_df$sample, levels=unique(sort(telo_master_df$sample)))
  telo_master_df$Var1 <- factor(telo_master_df$Var1, levels=c(10500, 9500, 8500, 7500, 6500, 5500, 4500, 3500, 2500, 1500, 500))
  
  telo_bar_hist <- ggplot(data=telo_master_df) +
  ...
  ggsave("sample_comparison.telo_length.pdf", device="pdf", plot=telo_bar_hist, width=12, height=10)
}
