...
if (args[2]){
  ...
  telo_length_boxplot <- ggplot(data=telo_stats) +
    geom_boxplot(mapping=aes(y=telo_length)) +
    ylab("Telomere Length") +
    theme_minimal() + theme(axis.text.x = element_blank(),
                            axis.title.y = element_text(size=20),
                            axis.text = element_text(size=15))
  
  ggsave("telo_length_boxplot.pdf", device="pdf", plot=telo_length_boxplot, width=5, height=10)
...
if (args[4]) {
  ...
  if (args[2]){
    telo_length_by_strand <- ggplot(data=telo_stats) +
      geom_histogram(mapping=aes(telo_length), binwidth=50) +
      facet_grid(strand ~ .) +
      geom_vline(data=filter(telo_stats, strand=="C"), aes(xintercept=mean(telo_stats[telo_stats$strand == "C",]$telo_length)), color='red', linetype="dashed") +
      geom_vline(data=filter(telo_stats, strand=="G"), aes(xintercept=mean(telo_stats[telo_stats$strand == "G",]$telo_length)), color='red', linetype="dashed") +
    ...
    ggsave("STRAND_COMPARISON/telo_Length.pdf", device="pdf", width=10, height=12, plot=telo_length_by_strand)
  
    telo_length_by_strand_boxplot <- ggplot(data=telo_stats) +
      geom_boxplot(mapping=aes(x=strand,y=telo_length)) + theme_minimal() +
      theme(axis.title = element_text(size=20),
            axis.text=element_text(size=15)) +
      xlab("Strand") + ylab("Telomere Length")
    
    ggsave("STRAND_COMPARISON/telo_length_boxplot.pdf", device="pdf", width=8, height=12, plot=telo_length_by_strand_boxplot)
    ...
    ggsave("STRAND_COMPARISON/telo_length_bar_plot.pdf", plot = telo_bar_hist, device="pdf", width=9, height=10, create.dir = TRUE)
